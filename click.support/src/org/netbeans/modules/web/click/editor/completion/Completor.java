...
public abstract class Completor {
    ...
    private volatile List<ClickCompletionItem> cache = new ArrayList<ClickCompletionItem>();
    ...
    public final CompletionResult complete(CompletionContext context) {
        try {
            anchorOffset = initAnchorOffset(context);
            compute(context);
            return CompletionResult.create(cache, anchorOffset, hasAdditionalItems, getAdditionalItemsText());
    ...
    public final CompletionResult filter(CompletionContext context) {
        return CompletionResult.create(doFilter(context), anchorOffset, hasAdditionalItems, getAdditionalItemsText());
    }
    ...
    protected final void addCacheItem(ClickCompletionItem item) {
        cache.add(item);
    }

    protected List<ClickCompletionItem> getCacheItems() {
        return cache;
    }
