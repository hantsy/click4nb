...
public final class CompletorUtils {
    ...
    public static List<ClickCompletionItem> filter(List<ClickCompletionItem> cachedItems,
            Document doc, int invocationOffset, int caretOffset, int anchorOffset) {
        if(anchorOffset == -1) {
            return Collections.emptyList();
        }
        
        List<ClickCompletionItem> filteredItems = new ArrayList<ClickCompletionItem>();
        
        try {
            String prefix = doc.getText(anchorOffset, caretOffset - anchorOffset);
            for (ClickCompletionItem item : cachedItems) {
            ...
            Exceptions.printStackTrace(ex);
            filteredItems = Collections.<ClickCompletionItem>emptyList();
        }
